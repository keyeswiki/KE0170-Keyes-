...
- Renamed ``USE_SOFT_CARRIER`` and ``USE_NO_CARRIER`` macros to
  ``USE_SOFT_SEND_PWM`` and ``USE_NO_SEND_PWM``.
...
- Added functions ``printResultShort(&Serial)`` and
  ``getProtocolString(decode_type_t aDecodeType)``.
...
- Formatting and changing ``TIMER_CONFIG_KHZ`` and
  ``TIMER_CONFIG_NORMAL`` macros to static functions.
...
- Changed rawlen from uint8_t to unsigned int allowing bigger receive
  buffer and renamed ``RAWBUF`` to ``RAW_BUFFER_LENGTH``.
- Introduced ``USE_NO_CARRIER`` for simulating an IR receiver. Changes
  from #283 by bengtmartensson
- Added function sendRaw_P() for sending data from flash. Changes from
  #268 by adamlhumphreys
